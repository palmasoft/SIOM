...
$upload_dir = '/archivos/afiliados/censo/'.$usuarioIMG;
...
if (isset($_FILES['upload']) && strlen($_FILES['upload']['name']) > 1) {
    ...
    if (!in_array($type, $imgsets['type'])) {
        $err .= 'Enviado: ' . $_FILES['upload']['name'] . '. tipo de archivo no permitido.';
    }
    if ($_FILES['upload']['size'] > $imgsets['maxsize'] * 1000) {
        $err .= '\\n El tamaño maximo del archivo deb ser de : ' . $imgsets['maxsize'] . ' KB.';
    }
    ...
    if ($err == '') {
        if (move_uploaded_file($_FILES['upload']['tmp_name'], $uploadpath)) {
            ...
            $message = $img_name . ' Cargado Correctamente: \\n- Size: ' . number_format($_FILES['upload']['size'] / 1024,
                                                                                         3,
                                                                                         '.',
                                                                                         '') . ' KB \\n- Image Ancho x Alto: ' . $width . ' x ' . $height;
            $re = "window.parent.CKEDITOR.tools.callFunction($CKEditorFuncNum, '$url', '$message')";
        } else {
            $re = 'alert("No se pudo cargar el archivo ' . ($_FILES['upload']['tmp_name']) . '  ")';
        }
    } else {
        $re = 'alert("' . $err . '")';
    }
