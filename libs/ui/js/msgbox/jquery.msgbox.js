        ...
        for (var part in style) {
            ...
            for (var name in cs) {
                if (legacy) {
                    switch (name) {
                        case 'opacity':
                            builder.push('filter:alpha(opacity=' + (+(cs[name].substring(0, 1) == '.' ? ('0' + cs[name]) : cs[name]) * 100 || 100) + ');');
                            break;
                        default:
    ...
    $.msgbox = function (msg, options) {
        ...
        var io = {},
                ...
                iae = function (e) { //포커스된 요소가 메시지박스 아니면 메시지박스로 강제 포커스 이동
                    setTimeout(function () {
                        ...
                        if (act.length && (act.is(ms[0]) || act.is(ms[1]))) {
                            ...
                            $C.find(ms + '').eq(0).focus();
                    }, 0);
                },
                $W = $(window),
                //경고창
                $C = $$(div).addClass(mb + 'container').append($$(div).addClass(mb + 'ui').addClass(mb + (p ? 'prompt' : (q ? 'confirm' : ( options.type ? options.type :'alert' ) ) ) ) ).children(),
                //경고창 배경
            ...
            switch (code) {
                ...
                case 39://오른쪽키 누르면 다음 버튼으로만 포커스
                    e.preventDefault();
                    if (target = that[code == 9 && shift ? 'prev' : 'next'](btn), target.length)
                        target.focus();
                    else if (code == 9) {
                        if (target = $C.find(dmb + 'input'), target.length)
                            target.select();
                        else if (target = that[shift ? 'next' : 'prev'](btn), target.length)
                            target.focus();
                    }
                    break;
                case 37://왼쪽키는 이전 버튼으로만 포커스
                    e.preventDefault();
                    if (target = that.prev(btn), target.length)
                        target.focus();
                    break;
        ...
        $C.delegate(btn, clk, function (e) {
            ...
            if ($.isFunction(onclose))
                $.when(onclose.call($C[0], p ? I.children().val() : $(this).hasClass(cok))).always(cleanup);
            else
                cleanup();
        });
        ...
        var prepare = function () {
            ...
            //경고창 포커스
            if (p)
                $C.find('input:text').select();
            else
                $C.find(btn + ':eq(0)').focus();
            //onopen 이벤트 발생
            var onopen = options.onopen || $.msgbox.onopen;
            if ($.isFunction(onopen))
                onopen.call($C[0], options);
        };
